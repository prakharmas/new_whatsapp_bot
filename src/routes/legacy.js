...
module.exports = function(app) {
    ...
    // JSON API route for React Wallet page
    app.get('/api/vendor/wallet', requireAuth, async (req, res) => {
        try {
            let wallet = await VendorWallet.findOne({ vendor_id: req.vendorId });
            if (!wallet) {
                wallet = new VendorWallet({ vendor_id: req.vendorId });
            ...
            // Usage summary by service (last 30 days)
            const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
            const recentUsage = await UsageRecord.find({
                vendor_id: req.vendorId,
                charged_at: { $gte: thirtyDaysAgo }
            ...
            // Daily spending for the last 7 days to estimate burn rate
            ...
            if (recentTransactions.length > 0) {
                const totalDebitINR = recentTransactions.reduce((sum, t) => sum + t.amount_inr, 0);
                dailyBurnINR = totalDebitINR / 7;
            }

            const balanceINR = wallet.balance_usd_micro / 1000000 * exchangeRate.rate;
            const daysRemaining = dailyBurnINR > 0 ? Math.round(balanceINR / dailyBurnINR) : null;

            res.json({
                success: true,
                data: {
                    wallet: {
                        balance_usd: wallet.balance_usd_micro / 1000000,
                        balance_inr: Math.round(balanceINR),
                        last_updated: wallet.last_updated
                    },
                    exchange_rate: exchangeRate.rate,
                    total_messages: totalMessages,
                    total_spent_inr: Math.round(totalSpentINR),
                    avg_cost_inr: parseFloat(avgCostINR.toFixed(2)),
                    daily_burn_inr: Math.round(dailyBurnINR),
                    ...
                    usage_summary: usageSummaryArray
                }
            });
        } catch (error) {
